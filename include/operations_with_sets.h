...
namespace operations_with_sets{
    ...
    /**
     *  \brief Converting a set into an std::string.
     *  \param [in] a          Converted set.
     *  \param [in] print_elem Function of a conversion of the set element.
     */
    template<typename T>
    std::string show_set(const std::set<T>& a, std::string (*show_elem)(const T&))
    {
        std::string result;
        if(a.empty()){
            result = "{}";
